...
$_kernel=$ker;
Giraf::Core::debug("Giraf::Modules::HaroBattle::init()");
...
get_betters(); # TODO: This is horrible, change this.
...
Giraf::Session::init_session("harobattle_core");
start_session();
}
...
Giraf::Chan::part($_chan, "Module déchargé");

Giraf::Session::shutdown_session("harobattle_core");
}
...
Giraf::Session::post_event('harobattle_core', 'harobattle_original', $dest, 1);
...
if ($_bets->{$uuid}->{wealth} < 20) {
	push(@return, linemaker("Soutenir un haro coûte 250, vous n'avez pas assez d'argent."));
	return @return;
}
...
sub root_reward {
	my ($colour) = @_;
	my $sth = $_dbh->prepare("SELECT count(*) FROM $_tbl_betters WHERE colour LIKE ?");
	my $count;
	...
	if ($count > 0) {
		foreach my $i (keys %$_bets) {
			my $chose = $_bets->{$i}->{colour};
			Giraf::Core::debug("root_reward : haro $colour, joueur $chose.");
			if ($_bets->{$i}->{colour} eq $colour) {
				Giraf::Core::debug("root_reward : le joueur palpe.");
				$_bets->{$i}->{wealth} += int($_cagnotte/$count);
			}
		}
		my $sth = $_dbh->prepare("UPDATE $_tbl_betters SET wealth = wealth + ? WHERE colour LIKE ?");
		$sth->execute(int($_cagnotte/$count), $colour);

		$_cagnotte = 0;
	}
#	push(@return, linemaker("roots mis a jour"));
#	return @return;
}
...
if ($_pot) {
	push(@return, linemaker("$_pot sont versés dans la cagnotte."));
	$_cagnotte += $_pot;
	$_pot = 0;
}
...
$kernel->delay_set('harobattle_initiative', 60, $dest, 4);
